...
* **display data on your watch**: by providing [custom watchfaces](Watchfaces.md#aaps-watchfaces) or in stardard watchfaces with use of complications
...
* If description of smartwatch indicates only compatibility with _Android_ and _iOS_ - it **does not** means it runs on _Wear OS_ - it may as well be some other sort of Vendor speciffic OS **which is not compatible with AAPS wear!**
...
Wear variant of APK need to be installed on watch in the same way you will install phone APK on phone. It may require enabling _developer mode_ on watch and uploading and installig APK on watch with: `adb install wear-full-release.apk`
...
* **Matching divder** (default Off): For AAPS, AAPSv2 and AAPS(Large) watchfaces, show contrast backround for divider (**Off**) or match divider with the background color (**On**)
...
* **Animations**: Whan enabled, on supported by watch and not in power saving low-res mode, watchface circle will be animated
