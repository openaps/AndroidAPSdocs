...
** For example, to whitelist on a Samsung phone running Android Pie: Go to Settings -> Device Care -> Battery -> Scroll until you find AndroidAPS and select it -> De-select "Put app to sleep". ALSO go to Settings -> Apps -> (Three circle symbol in the top-right of the screen) select "special access" -> Optimize battery usage -> Scroll to AndroidAPS and make sure it is de-selected.
