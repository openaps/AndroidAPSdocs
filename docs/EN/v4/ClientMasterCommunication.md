...
Tap the **+** button to add a client. Enter a **device name** (so you can recognise it in the list), and the master will show a **pairing QR code** together with a short **PIN**. This offer is **one-time** and **expires after about two minutes**.

<!-- 📷 SCREENSHOT NEEDED: master pairing screen showing the QR code + PIN.
     Use a DEMO/expired code only — the QR contains a PIN-wrapped pairing secret and must not be a live one. -->

```{admonition} The QR / PIN is a secret
:class: warning
The QR code carries the pairing secret (wrapped with the PIN). Show it only to the device you are pairing, do not post it publicly, and let it expire if you do not use it. A fresh code can always be generated by tapping **+** again.
```

(client_master_client_setup)=
### 2. On the client — pair with the master

On the client, open **Manage → Pair with master**, then **scan the QR** shown on the master (or enter the **PIN** manually).
...
:class: tip
- **Pairing secret** is exchanged once via the QR/PIN and stored encrypted on each device; it never travels through Nightscout in clear text.
...
<!-- =====================================================================
     SCREENSHOTS STILL TO ADD (placeholders marked inline above):
       1. Master pairing screen: QR code + PIN  (use a demo/expired code!)
       2. Client "Pair with master" in the UNPAIRED state (scan QR / enter PIN)
