...
Section **3) Daily life with AAPS** covers key **AAPS** features, to help you use (and customize) **AAPS**. This includes understanding the screens, carbs-on-board, sensitivity, profile switching, temp targets, extended carbs (or eCarbs), automations, and DynamicISF. It also covers frequent topics like how to manage different types of meals, how to deal with cannula and sensor changes, smartphone updates, daylight saving changes, [traveling with AAPS](DailyLifeWithAaps/TimezoneTraveling-DaylightSavingTime.md), and sports. Common questions and answers are located within the troubleshooting section. 
...
```{toctree}
:caption: 3) Daily Life with AAPS
...
```
```{toctree}
:caption: 5) Wear OS Smartwatches
...
```{toctree}
:caption: 7) Help & Troubleshooting
...
```{toctree}
:caption: 8) Advanced AAPS options
