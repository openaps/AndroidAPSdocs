...
Users are strongly encouraged to keep their build of AndroidAPS up to date for safety reason, However for users unable to use a device with a minimum version of Android 8.0, a build (2.6.1.4) suitable for older Android versions remains available from the `old repo. <https://github.com/miloskozak/androidaps>_`
