...
- The Dana*R/RS connects to almost any phone with Android >= 5.1 without the need to flash lineage. If your phone breaks you usually can find easily any phone that works with the Dana*R/RS pumps as quick replacement... Mit der Combo ist das nicht so einfach,  jedenfalls nicht solange Android 8.1 nur auf wenigen Smartphones installiert ist.
- Initial pairing is simpler with the Dana* RS. Allerdings ist dieser Schritt normalerweise nur bei der Ersteinrichtung erforderlich.
...
- The Combo vibrates on the end of TBRs, the Dana* R vibrates (or beeps) on SMB. In der Nacht wird der Loop meistens eher TBR setzen statt SMB.  Die DanaRS kann so eingestellt werden, dass sie weder bei TBR, noch bei SMB vibriert oder piept.
