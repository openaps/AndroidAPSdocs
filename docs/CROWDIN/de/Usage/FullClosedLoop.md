...
**Achtung: Automatisierung #3 funktioniert nur, wenn kein aktives TT aktiv ist.** Also, falls Du mit einem temporären Ziel "Bald Essen" gearbeitet hast, muss diese bis zum Start der Automation beendet sein (d. h. normalerweise 30-40 Minuten nach Beginn der Mahlzeit).
...
> Möglichkeiten ein "Bald Essen"-TT zu nutzen Um zu Mahlzeitbeginn einen niedrigen Ausgangs-Glukosewert und ein wenig aktives Insulin (IOB) zu haben, stellen einige Looper durch drücken der "Bald Essen"-Schaltfläche (oder bei verlässlichen Essenszeiten: automatisch über ein reduziertes Profil-Glukoseziel) mindestens eine Stunde vor Beginn der Mahlzeit ein "Bald Essen"-TT ein. Aber, unter der Annahme, dass der FCL sowieso immer auf dem Weg zum Ziel ist, könnte dies nicht viel bringen und Du kannst stattdessen einfach eine Automatition definieren, die beim Erkennen des Mahlzeitenbeginns (Glukose-Delta oder Beschleunigung = Delta > Durchschnitts-Delta) ein "Bald Essen"-TT setzt. Ein niedriges temporäres Ziel ist in dieser Phase wichtig, da jeder SMB des Loops durch (vorhergesagter Glukose minus temporäres Ziel)/Sensititvität berechnet wird. Damit ist ein niedriges temporäres Ziel treibend für den daraus resultierenden Insulinbedarf. größer.
...
Automation #5, “Hoch nach dem Essen”, ist auch für einen Hybrid Closed Loop anwendbar
...
- Lege den IOB-Schwellwert, ab dem der Loop deutlich vorsichtiger gemacht wird (s. Automatisierung #3 oben), fest. Beachte, dass, wenn der Loop den Bedarf für zusätzliches Insulin (insulinReq) sieht, dieser IOB durch den noch nicht wirksamen letzten SMB und dann folgende temporäre Basalraten (TBRs) überschritten werden kann. Absorbierte Kohlenhydrate werden eine Gegenbewegung in Richtung eines niedrigeren IOB bringen.
