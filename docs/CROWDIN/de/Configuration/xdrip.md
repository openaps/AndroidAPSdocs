...
* Achtung: Die Auto-Korrektur neigt manchmal dazu, das i von info in einen Großbuchstaben zu ändern. Du **darfst nur Kleinbuchstaben** verwenden, wenn Du ` info.nightscout.androidaps ` eingibst. Capital I would prevent AAPS from receiving BG values from xDrip+.
...
* Forget device in xDrip+ system status AND in smartphone’s BT settings (Will be shown as Dexcom?? dabei steht ?? für die letzten beiden Zeichen der Seriennummer Deines Transmitters).
