...
- Instalar o AndroidAPS como descrito em [AndroidAPS wiki](http://wiki.AndroidAPS.org) e usar a opção `combo`.
...
- Siga o link http://ruffy.AndroidAPS.org e copie ruffy via git. Use o mesmo branch que usava para AndroidAPS, agora é `combo`, mais tarde serão `master` e `dev`.
