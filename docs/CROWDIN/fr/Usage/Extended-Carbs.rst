...
C'est pourquoi à partir de la version 2.6, il y a une option pour un bolus étendu pour les utilisateurs de pompes Dana et Insight. On Insight pump extended bolus is not available if `TBR emulation <../Configuration/Accu-Chek-Insight-Pump.html#settings-in-aaps>`_ is used. For Dana and Insight pump closed loop will automatically be stopped and switched to open loop mode for the time running extended bolus. La quantité d'insuline, le temps restant et le temps total seront affichés sur la page d'accueil.
