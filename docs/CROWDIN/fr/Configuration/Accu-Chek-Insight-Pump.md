...
Si vous avez besoin d'utiliser autotune, la seule solution pour le moment est de désactiver la synchronisation avec Nightscout si vous avez besoin d'utiliser autotune : Dans AAPS, allez dans Paramètres / NSClient / Paramètres Avancés et activez **Remonter uniquement vers NS (sync désactivée)**.
