...
* Первичное заполнение инфузионной системы(если поддерживается помпой [ DanaR/RS, Combo и Insight])
* Журнал
