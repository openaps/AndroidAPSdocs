...
- **Задать нейтральный временный базал** -это вариант, который предотвратит ежечасную подачу сигнала помпой Medtronic. If enabled if will cancel a temp basal before the hour end to prevent this from happening.
