...
<ol start="1">
  <li>
    עברו ללשונית <strong x-id="1">פעולות (ACT)</strong> מהמסך הראשי של AAPS.
  </li>
  
  <li>
    Under the <strong x-id="1">Careportal (1)</strong> section the following 3 fields will have their <strong x-id="1">age reset</strong> to 0 days and 0 hours <strong x-id="1">after each pod change</strong>: <strong x-id="1">Insulin</strong> and <strong x-id="1">Cannula</strong>. זה נעשה בגלל האופן שבו משאבת אומניפוד בנויה ופועלת. <strong x-id="1">סוללת המשאבה</strong> ו<strong x-id="1">יתרת האינסולין</strong> נמצאים בעצמם בתוך כל פוד. מכיוון שהפוד מחדיר את הצינורית ישירות לעור באתר הדבקתו, אין שימוש בצינור מסורתי במשאבות אומניפוד. <em x-id="3">לכן לאחר החלפת פוד, הגיל של כל אחד מהערכים הללו יתאפס אוטומטית.</em> <strong x-id="1">גיל סוללת המשאבה</strong> אינו מדווח מכיוון שהסוללה בפוד תמיד תהיה יותר מאורך חיי הפוד (80 שעות לכל היותר).
  </li>
</ol>
...
<p spaces-before="0">
  <strong x-id="1">יתרת אינסולין</strong>
</p>
...
<blockquote spaces-before="0">
  <ul>
    ...
    <li>
      <strong x-id="1">SMS</strong> - מחזירה ערך או 50+U בתגובות של SMS
    </li>
    <li>
      <strong x-id="1">נייטסקאוט</strong> - העלאת ערך של 50 כאשר מעל 50 יחידות לנייטסקאוט (גרסה 14.07 ומעלה).  גרסאות חדשות יותר ידווחו על ערך של 50+ כאשר מעל 50 יחידות.
    </li>
  </ul>
</blockquote>
...
<p spaces-before="0">
  פודים נכשלים מדי פעם בשל מגוון בעיות, כולל בעיות חומרה של הפוד עצמו. הנוהג הטוב ביותר הוא לא להודיע אותם למשווק המשאבה, מכיוון ש- AAPS אינו מקרה שימוש מאושר. A list of fault codes can be found <a href="https://github.com/openaps/openomni/wiki/Fault-event-codes">here</a> to help determine the cause.
</p>
...
<p spaces-before="0">
  כשל זה קשור למצב פוד שגוי עבור פקודה או שגיאה במהלך פקודת אספקת אינסולין. We recommend users to switch to the Nightscout client to <em x-id="3">upload only (Disable sync)</em> under the <strong x-id="1">Config Builder</strong>➜<strong x-id="1">General</strong>➜<strong x-id="1">NSClient</strong>➜<strong x-id="1">cog wheel</strong>➜<strong x-id="1">Advanced Settings</strong> to prevent possible failures.
</p>
...
<p spaces-before="0">
  שימו לב כי לייבוא הגדרות יש אפשרות לייבא סטטוס פוד מיושן. כתוצאה מכך, אתם עלולים לאבד פוד פעיל. It is therefore strongly recommended that you <strong x-id="1">do not import settings while on an active Pod session</strong>.
</p>

<ol start="1">
  <li>
    בטלו את הפעלת הפוד. ודאו שאין פוד פעיל.
  </li>
  
  <li>
    ייצאו את ההגדרות שלך ושמרו עותק במקום בטוח.
  </li>
  
  <li>
    הסירו את ההתקנה של הגרסה הקודמת של AAPS והפעל מחדש את הטלפון.
  </li>
  
  <li>
    התקינו את הגרסה החדשה של AAPS וודאו כי אין פוד בפעולה.
  </li>
  
  <li>
    ייבאו את ההגדרות והפעילו את הפוד החדש.
  </li>
</ol>
...
<p spaces-before="0">
  please note that the Omnipod driver presents a variety of unique alerts on the <strong x-id="1">Overview tab</strong>, most of them are informational and can be dismissed while some provide the user with an action to take to resolve the cause of the triggered alert. להלן סיכום של ההתראות העיקריות בהן אתם עשויים להיתקל:
</p>
...
<p spaces-before="0">
  לא זוהתה הפעלת פוד פעילה. ניתן לבטל התראה זו זמנית על ידי לחיצה על <strong x-id="1">השתקה</strong> אך היא תמשיך לפעול כל עוד לא הופעל פוד חדש. לאחר הפעלת פוד, ההתראה הזו מושתקת אוטומטית.
</p>
...
<p spaces-before="0">
  התראה אינפורמטיבית על כך שהגדרת הפרופיל הבזאלי של פוד נכשלה, ויהיה עליכם ללחוץ על <em x-id="3">רענן</em> בלשונית DASH.
</p>
...
<p spaces-before="0">
  Due to the way that the RileyLink and Omnipod communicate, situations can occur where it is <em x-id="3">uncertain</em> if a command was successfully processed. לכן יש צורך ליידע את המשתמש על אי וודאות זו כשהכרחי.
</p>
...
<ul>
  <li>
    <strong x-id="1">Boluses</strong> - Uncertain boluses cannot be automatically verified. ההודעה תישאר עד הבולוס הבא אך רענון תרמיל ידני ינקה את ההודעה. <em x-id="3">By default alerts beeps are enabled for this notification type as the user will manually need to verify them.</em>
  </li>
  <li>
    <strong x-id="1">TBRs, Pod Statuses, Profile Switches, Time Changes</strong> - a manual pod refresh will clear the message. כברירת מחדל, צפצופי התראה מושבתים עבור סוג התראה זה.
  </li>
  <li>
    <strong x-id="1">Pod Time Deviation -</strong> When the time on the pod and the time your phone deviates too much then it is difficult for AAPS loop to function and make accurate predictions and dosage recommendations. אם סטיית הזמן בין התרמיל לטלפון היא יותר מחמש דקות אז AAPS תדווח HANDLE TIME CHANGE תחת מצב פוד על כך שהפוד נמצא במצב מושעה. An additional <strong x-id="1">Set Time</strong> icon will appear at the bottom of the Omnipod (POD) tab. לחיצה על הגדרת זמן תסנכרן את השעון שעל הפוד עם השעה בטלפון ולאחר מכן תוכלו ללחוץ על כפתור חידוש הזרקה להמשך פעולות הפוד הרגילות.
  </li>
</ul>
...
<ul>
  <li>
    <strong x-id="1">רמה 0</strong> קראו את הסעיף הרלוונטי בתיעוד זה כדי להבטיח שאתם מבינים כיצד להשתמש במה שאתם מתקשים איתו.
  </li>
  <li>
    <strong x-id="1">רמה 1:</strong> אם אתם עדיין נתקלים בבעיות שאתם לא מצליחים לפתור באמצעות מסמך זה, אנא עברו לערוץ <em x-id="3">#androidaps</em> ב-<strong x-id="1">Discord</strong> באמצעות <a href="https://discord.gg/4fQUWHZ4Mw">קישור ההזמנה</a>.
  </li>
  <li>
  ...
  <li>
    <strong x-id="1">היו סבלניים - רוב חברי הקהילה שלנו מורכבים ממתנדבים בעלי אופי טוב, ופתרון בעיות דורש לעתים קרובות זמן וסבלנות מצד המשתמשים והמפתחים כאחד.</strong>
  </li>
</ul>
