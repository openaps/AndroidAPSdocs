...
本 **AAPS** 文档侧重于 **AAPS** 开发项目的** "主 （master）"**分支。 然而，研发工作一直在进行。 更有经验的用户可能希望探索**开发分支**中的实验功能。 这包括集成 Dexcom G7 ，以及根据短期敏感性变化自动调整胰岛素给药量（DYNISF）。 开发创新的重点是全闭环策略（不必输注餐前大剂量_等_），总体而言，就是要尽可能方便 1 型糖尿病患者的生活。
