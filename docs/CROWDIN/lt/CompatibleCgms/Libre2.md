...
For legal reasons, "patching" has to be done by yourself. Naudokite paieškos sistemas, norėdami rasti atitinkamas nuorodas. There are two variants: The recommended original patched app blocks any internet traffic to avoid tracking. The other variant supports LibreView.
...
Pirmas ryšio nustatymas su jutikliu yra ypač svarbus. The LibreLink app tries to establish a wireless connection to the sensor every 30 seconds. Jei trūksta vieno ar kelių privalomų parametrų, jie turi būti koreguojami. Galite tai atlikti bet kuriuo metu. The sensor is constantly trying to setup the connection. Net jei tai trunka keletą valandų. Be patient and try different settings before even thinking of changing the sensor.
