...
Libre 傳感器的干擾較大，可能需要平滑處理。 在使用 xDrip+ 直接連線或修補的應用程式資料來源（接收來自其他應用程式，包括 Juggluco）時，[已在應用程式內進行平滑處理](#libre2-value-smoothing-raw-values)。

| 傳感器 / 資料來源      | Juggluco | xDrip+ 直接 | xDrip+ 橋接 | xDrip+ 修補應用程式 |
| --------------- |:--------:|:---------:|:---------:|:-------------:|
