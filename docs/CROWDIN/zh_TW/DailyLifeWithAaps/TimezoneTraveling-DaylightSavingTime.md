...
Insight 也會紀錄更改時間的歷史條目，紀錄時間從哪個（舊）時間更改為哪個（新）時間。 因此，儘管時間發生變更，仍可以在 AAPS 中確定正確的時間。
...
根據幫浦和 CGM 的設置，時間跳變可能會導致問題。 以 Combo 為例，幫浦的歷史紀錄將重新讀取，並會導致重複條目。 因此，請在白天而非晚上進行時間調整。
