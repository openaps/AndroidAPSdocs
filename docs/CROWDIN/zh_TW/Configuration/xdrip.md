...
- 在 xDrip+ 中啟動傳感器。
    ...
    → 漢堡選單 (1) → 啟動傳感器 (2) → 啟動傳感器 (3) → 設置你使用讀取器或供應商應用程式啟動傳感器的精確時間。如果你不是今天啟動的，請選擇“不是今天” (4)。
...
- 你可以在不常見設定->額外日誌記錄設定->額外標籤記錄中新增`BgReading:d,xdrip libre_receiver:v`。 這將記錄額外的錯誤訊息以進行故障排除。
