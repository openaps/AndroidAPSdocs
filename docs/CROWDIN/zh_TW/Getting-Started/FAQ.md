...
如果你定期在同一時間進行運動（例如在健身房上運動課），你可以考慮使用[自動化](../Usage/Automation.md)來進行設定檔切換和臨時目標的設置。 基於位置的自動化也可能是一個不錯的做法，但會讓前置處理變得更複雜。
...
使用 Dana R 和 Dana R Korean 時，你不需要做任何操作。 對於其他幫浦，請參閱[時區旅行](../Usage/Timezone-traveling.md)頁面以了解更多詳細訊息。
