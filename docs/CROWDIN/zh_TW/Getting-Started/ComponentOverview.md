...
即使你不需要使用 [xDrip+ 應用程式](https://xdrip.readthedocs.io/en/latest/) 作為 **BG 資料源**，你仍然可以使用它來設定 _例如_ 警報或不同的血糖顯示。 你可以設定任意多的警報，指定警報應該在什麼時間啟動，是否可以覆蓋靜音模式等。 有關一些 xDrip+ 資訊，請參閱[這裡](../CompatibleCgms/xDrip.md)。 請注意，這款應用程式的文件並非總是最新的，因為其進展相當快速。
