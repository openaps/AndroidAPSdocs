...
* **分隔線**（預設`關`）：對於 AAPS、AAPSv2 和 AAPS（大）手錶外觀，顯示分隔線的對比背景（**關**）或分隔線與背景顏色（**開**）。
...
自 AAPS 版本3開始，我們可以在永遠開啟模式下使用“簡化 UI”。 此 UI 只包含血糖值、方向和時間。 This UI is power-optimized with less frequent updates, showing less information and lighting up fewer pixels to save power on OLED displays.
...
* 嘗試為AAPS手錶面使用**深色**主題，以及[**分隔符**](#watchface-settings)。 在 OLED 裝置上，這將限制點亮的像素數量並減少燒灼風險。
