...
某些選項可能會被灰階化，這是因為手機設置為兒童帳戶，並需要透過家庭帳戶管理員啟用。 在照護者/父母的 iPhone 上，這個日期和時間設置稱為「自動設置」。 如果你不確定手機是否已同步，請不要擔心，你可以先設置簡訊指令，若發現問題可以再進行故障排除（如有需要請尋求幫助）。
...
如果你沒有收到任何回應，請檢查下方的 [故障排除](#SMSCommands-troubleshooting) 區段。

2) 現在試試需要驗證器的簡訊指令，例如 “target hypo”。 照護者的手機會收到一條回覆簡訊，提示你輸入來自驗證器應用程式的**六位數驗證碼**，然後是消防氣 **僅藉由照護者/跟隨者所知的額外秘密 PIN**（總共十個數字的字串，假設你的 PIN 只有 4 位數）。
...
如果你的指令成功，你將收到一則確認回覆。 如果出現問題，你將收到錯誤訊息。 請參見下方的[故障排除](#SMSCommands-troubleshooting)以獲得常見錯誤。
...
| 指令         | 驗證 | 功能 & *回應*                                                                                    |
| ---------- | -- | -------------------------------------------------------------------------------------------- |
| 血糖值        | 否  | 返回：最後血糖、變化、IOB（注射和基礎）、COB<br/>*最後血糖：5.6 4分前，變化：-0.2 mmol，IOB：0.20U（注射：0.10U 基礎：0.10U）* |
...
| 指令          | 驗證 | 功能 & *回應*              |
| ----------- | -- | ---------------------- |
| 基礎注射 0.3    | 是  | 開始基礎注射 0.3U/h 持續 30 分鐘 |
| 基礎注射 0.3 20 | 是  | 開始基礎注射 0.3U/h 持續 20 分鐘 |
| 基礎注射 30%    | 是  | 開始基礎注射 30% 持續 30 分鐘    |
| 基礎注射 30% 50 | 是  | 開始基礎注射 30% 持續 50 分鐘    |
| 停止/取消基礎注射   | 是  | 停止臨時基礎注射               |
...
| 指令                 | 驗證 | 功能 & *回應*                                                                                                     |
| ------------------ | -- | ------------------------------------------------------------------------------------------------------------- |
| LOOP STATUS (循環狀態) | 否  | 反應依賴於實際狀態：<br/> - *當循環關閉時，循環已停用*或 LGS<br/> - *當循環開啟或閉合時，循環已啟用*<br/> - *暫停 (10 分鐘)*當循環被中斷或暫停 |
| 循環停止/停用            | 是  | 幫浦將恢復到預設的基礎注射速率。<br/>*循環已被停用*                                                                           |
| 循環開始/啟用            | 是  | *循環已啟用*                                                                                                       |
| 循環暫停 20            | 是  | *循環暫停 20 分鐘*                                                                                                  |
| 循環恢復               | 是  | *循環已恢復*                                                                                                       |
| 循環閉合               | 是  | *當前循環模式：閉合循環*                                                                                                 |
| 循環 LGS             | 是  | *當前循環模式：低血糖暫停*                                                                                                |
...
| -------------------- | -- | ---------------------------------------------------------------------------------- |
| 注射 1.2               | 是  |                                                                                    |
| 注射 0.60 餐前注射         | 是  | 輸送指定的 0.60U 注射<br/>**並** 設定 [即將用餐臨時目標](#TempTargets-eating-soon-temp-target) |
| 碳水化合物 5              | 是  | 輸入 5g，無需注射                                                                         |
| 碳水化合物 5 17:35/5:35PM | 是  | 在 17:35 輸入 5g。<br/>可接受的時間格式依賴於<br/>手機上的時間設定 (12 小時/24 小時)。             |
| 延長注射 2 120           | 是  | 開始延長注射 2U，持續 120 分鐘。<br/>僅限於 [相容的幫浦](#screens-action-tab)。                   |
| 延長停止/取消              | 是  | 停止延長注射                                                                             |
...
| --------- | -- | ----------------------------------------------------------------------- |
| 個人設定狀態    | 否  | 當前個人設定與百分比                                                              |
| 個人設定列表    | 否  | 當前 **AAPS** 中的個人設定列表，例如：<br/>1. 個人設定1<br/> 2. 個人設定2         |
| 個人設定 1    | 是  | 切換到列表中的個人設定 1。<br/>使用由 **個人設定列表** 返回的數字，<br/>而不是你儲存的個人設定名稱。 |
| 個人設定 2 30 | 是  | 切換到個人設定2 30%                                                            |

### 臨時目標

| 指令          | 驗證 | 功能 & *回應*        |
| ----------- | -- | ---------------- |
| 目標餐點/活動/低血糖 | 是  | 設置臨時目標 餐點/活動/低血糖 |
| 目標停止/取消     | 是  | 取消臨時目標           |
...
| 指令          | 驗證 | 功能 & *回應*                                                                                       |
| ----------- | -- | ----------------------------------------------------------------------------------------------- |
| 治療 重新整理     | 否  | 從 NS 重新整理治療                                                                                     |
| NSCLIENT 重啟 | 否  | 如果你注意到與 Nightscout 或 **AAPSClient** 的通訊問題，這將很有幫助。                                               |
| 停用/停止簡訊     | 否  | 要停用 SMS 遠端服務，請回覆代碼 Any。<br/>請注意，你可以直接從 **AAPS** 主智慧型手機重新啟用該服務。                            |
| 協助          | 否  | 返回所有可查詢的功能：<br/>BG、循環、治療等。<br/>發送進一步的 ***協助 ***功能****** 命令以列出<br/>此區域內所有可用選項。 |
| 協助注射        |    | *注射 1.2<br/>注射 1.2 餐前*                                                                    |
