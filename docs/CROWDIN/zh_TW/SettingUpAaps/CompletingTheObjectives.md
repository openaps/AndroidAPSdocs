...
- 在 **總覽** 螢幕右上角長按 [循環圖示](#AapsScreens-loop-status)，選擇「開環」。
...
這**目標**要求你確定並設置你的「最大U/h的臨時基礎率」（max-basal）值，如[OpenAPS特徵](#Open-APS-features-max-u-h-a-temp-basal-can-be-set-to)所述。 此數值可在 [偏好設定 > OpenAPS SMB](#Preferences-openaps-smb-settings) 中設定。 如果你仍在使用虛擬幫浦，請確保在**AAPS**和你的胰島素幫浦中均設置此安全設置。
...
- 若你的 IOB 為負值（請見下方螢幕截圖），則可在 **目標 6** 中觸發高於 100% 的臨時基礎率（TBR）。
...
要完成**目標 7**，你必須關閉循環並提升你的[最大IOB](#Open-APS-features-maximum-total-iob-openaps-cant-go-over)。 由於低血糖暫停模式，**maxIOB** 在 **目標 6** 中會自動被歸零。 現在已不再如此。 **AAPS**將開始使用你所定義的最大IOB值來調整高血糖值。

完成這個目標的最短時間為**1天**。 這是一個強制的等待時間。 在這段時間內，無法進入下一個**目標**。

- 在 **總覽** 螢幕右上角長按 [循環圖示](#AapsScreens-loop-status)，選擇 **閉環**。 保持在**閉合循環**中至少持續1天。
