...
- 耐受幾分鐘的延遲第一餐注射，同時不會產生不可接受的峰值高度；並且
- 最小化不同碳水化合物負荷（餐點大小）對 **血糖** 峰值的影響。

**全閉環(FCL)** 不太可能對除 Lyumjev 或 Fiasp 之外的胰島素有效，除非用戶在非常適度或低碳水化合物飲食中。
...
- 如何以及在何處進行任何 **CGM** 平滑處理，以及這可能對您的調整有何影響。 特別是如何定義增量，以及 **AAPS** 將其識別為開始用餐的跡象。
- 幫浦和 CGM 幫浦的藍牙穩定性；
- 避免（或至少早期識別）幫浦阻塞；
...
不規範消費的零食和含有快速吸收碳水化合物的甜飲料對 **全閉環(FCL)** 是一個問題。
...
不尋常或不穩定的運動活動水平對 **全閉環(FCL)** 造成困難。 在運動前做好計畫是必要的（尤其是如果您想降低在運動期間需要急救碳水化合物/零食的需求）。 在經過一個活躍的日子後，建議在晚上餐點消化完全後設定較低的 **比例設定檔**：在 **自動化** 中設定一個較高（>100 mg/dl）的 **血糖** 目標，並選擇“No **SMBs** at elevated target” 在 **AAPS*** 偏好設定中。

#### 兒童的挑戰

**全閉環(FCL)** 對孩子來說可能帶來額外挑戰，包括：

- Lyumjev 或 Fiasp 可能無法提供或難以耐受。
- 每小時基礎率可能非常低，為大劑量 **SMBs** 提供不良的基礎。
- 飲食可能富含甜味成分。 隨著小身體的典型低血容量，容易出現非常高的 **血糖** 峰值。
...
擁有不穩定數據的**CGM**使得使用者處於困境，因為為了安全起見，您需要「沙包」您的定義，確保哪個變化肯定是開始用餐的跡象。 這意味著：

- **全閉環(FCL)**損失額外時間，結果是較高的**血糖**峰值和較低的%**TIR**；
...
在準備 **全閉環(FCL)** 時，使用者必須仔細檢查典型餐點的 **iob** 隨時間的變化，並判斷 **何時變得過多，以及你如何通過調整你的自動化來捕捉到這一點**。 這是可能的，因為我們有幾個調整螺絲。 正確做到這一點可能是一個挑戰。
...
> 如果你準備使用 AAPS 開發版本，你也可以使用允許進一步擴展 SMB 劑量的版本。 有些使用者也會在他們的「完全閉環」中使用小劑量的預注射。 然而，這會干擾葡萄糖曲線及其上升檢測和觸發的 **SMB** 的行為，因而在整體效益上並不易於實施。
