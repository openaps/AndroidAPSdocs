...
* 是否有多個來源將您的血糖數據上傳到 NS？ 如果您使用 BYODA 應用程式，請在 AAPS 中啟用上傳功能，但不要在 xDrip+ 中啟用它，如果您使用的是該應用程式。
* 是否有追蹤者接收了您的血糖數據，但又將其重新上傳到您的 NS 網站？
...
* 由於傳感器更換會中斷血糖數據的連續傳輸，更換傳感器後的黃色警告標誌是正常現象，無需擔心。
* 對於 Libre 使用者的特別注意事項：
...
* 碳水化合物通知比注射計算器的通知更為精確。 您可能會看到碳水化合物建議，而注射計算器不顯示缺少的碳水化合物。
...
* 可以在不使用[注射計算器](Screenshots-bolus-wizard)的情況下注射特定數量的胰島素。
...
![注射向導](../images/Home2020_BolusWizard_v2.png)
...
* 對於未來的碳水化合物，您可以選中提醒框（當輸入將來的時間時，此框將默認選中），這樣您可以在未來的某個時間得到進食碳水化合物的提醒。
   
   ![帶有進食提醒的注射向導](../images/Home2021_BolusWizard_EatingReminder.png)

### J 區

* 超級注射是將接下來兩小時的基礎胰島素添加到即時注射中，並發出 0% 暫時基礎率（TBR），以收回額外的胰島素。 此選項僅在[向導中啟用超級注射](Preferences-superbolus)時顯示，可在[偏好設定總覽](Preferences-overview)中設置。
...
* 出於安全原因，如果您希望注射向導基於現有的暫時目標進行計算，則**必須手動勾選 TT 框**。
...
* 如果您在使用注射向導後看到上述警告，則表示 AAPS 檢測到計算的 COB 值可能有誤。
...
* 血糖檢查、導管填充、傳感器插入和幫浦電池更換是[N 區](Screenshots-careportal-section-n)顯示數據的基礎。
...
* 如果您使用了[注射向導](Screenshots-bolus-wizard)來計算胰島素劑量，您可以稍後在 ts 標籤中審查此計算。
