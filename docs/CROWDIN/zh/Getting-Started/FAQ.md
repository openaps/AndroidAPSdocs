...
如果您使用的是 Dexcom G5 或 G6 以外的 CGM/FGM 且在 xDrip 原生模式中運行，您將在 AAPS OpenAPS 選項卡中收到此提示。 詳情請參閱[xDrip 中的血糖資料平滑處理](../Usage/Smoothing-Blood-Glucose-Data-in-xDrip.md)。
...
然而，更換插管時並不使用幫浦的“灌注輸液組”功能，而是使用一個不會顯示在注射歷史記錄中的注射來填充輸液組或插管。 這意味著它不會中斷當前運行的臨時基礎率。 在操作選項卡上，使用[灌注/填充按鈕](CPbefore26-pump)來設置填充輸液組所需的胰島素量並開始灌注。 如果這個量不足，重複填充操作。 您可以在偏好設定 > 其他 > 填充/灌注標準胰島素量中設置預設量按鈕。 請參考插管盒中的說明手冊，了解根據針頭長度和管路長度應灌注多少單位。
...
讓所有這些部件一起運作的最簡單方法是：1/ 從 AAPS 中刪除連接設備2/ 關閉連接設備3/ AAPS 三點選單，退出 AAPS4/ 長按 AAPS 圖示，進入 Android 選單，選擇 AAPS 應用程式資訊，強制停止 AAPS，然後刪除快取記憶體（不要刪除主要記憶體！）4bis/ 少數手機可能需要在這裡重新啟動。 您可以嘗試不重啟。 5/ 打開連接設備6/ 啟動 AAPS7/ 前往 Pod 標籤，三點選單，搜尋並連接連接設備
...
AAPS 沒有運行在 "開發者模式"。 AAPS 顯示以下訊息："運行開發版本。 閉環模式已停用"。
...
紅色三角形表示您有重複的血糖資料，AAPS 無法精確計算變化值。 您無法啟動閉環。 您需要刪除每個重複資料中的一個血糖資料，才能清除紅色三角形。 進入 BYODA 或 xDrip 標籤，長按您想刪除的一行，檢查重複的每一行，然後刪除（或者通過三點選單並選擇刪除，具體取決於您使用的 AAPS 版本）。 如果重複的血糖資料過多，您可能需要重置 AAPS 資料庫。 在這種情況下，您將會失去統計資料、IOB、COB 和選定的設定檔。
...
1) 暫停 DASH 幫浦。 這確保當 DASH 失去連接時，沒有正在運行或排隊的命令。2) 將手機切換到飛行模式以停用藍牙（以及 WiFi 和行動網路）。 這樣可以確保 AAPS 和 DASH 無法通信。 3) 匯出設定（包括 DASH 狀態）4) 從手機上複製剛匯出的設定文件（由於手機處於飛行模式且我們不想更改該狀態，最簡單的方法是使用 USB 連接線）。 5) 將設定文件複製到另一個手機上。 6) 在替代手機上的 AAPS 中匯入設定。 7) 檢查 DASH 標籤，確認它是否識別到 Pod。 8) 取消暫停 Pod。
