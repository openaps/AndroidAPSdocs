...
所以，請慢慢來配置您的循環，運行良好的 **AAPS** 循環帶來的好處是巨大的。
...
這只是歡迎訊息，您可以通過點擊“下一步”按鈕跳過：

![image](../images/setup-wizard/Screenshot_20231202_125636.png)

### 許可協議

在最終用戶許可協議中，有關於使用 **AAPS** 的法律方面的重要信息。 請仔細閱讀。
...
在此畫面中，我們提供更多背景信息，將更技術性的內容轉換為常用語言，或解釋其原因。

請點擊“下一步”按鈕：

![image](../images/setup-wizard/Screenshot_20231202_125709.png)

智慧手機的電池消耗仍然是需要考慮的因素，因為電池性能仍然相當有限。 因此，智慧手機上的Android操作系統在允許應用程式運行和消耗CPU時間（因此消耗電池電量）方面有嚴格的限制。

然而，**AAPS** 需要定期運行，_例如_ 每隔幾分鐘接收一次血糖讀數，然後應用演算法根據您的設定決定如何處理血糖水平。 因此，Android必須允許它這樣做。
...
雖然禁用社交媒體應用程式的通知是一個很好的功能，但您必須允許 **AAPS** 向您發送通知。
...
Android將藍牙通信的使用與位置服務的使用權限相關聯。 您可能在其他應用程式中也見過這一點。 如果您想訪問藍牙，通常需要位置權限。

**AAPS** 使用藍牙與您的CGM和胰島素幫浦進行通信，前提是它們直接由 **AAPS** 控制，而不是通過 **AAPS** 使用的其他應用程式。 具體細節可能因設置而異。
...
**AAPS** 需要將信息記錄到智慧手機的永久儲存中。 永久儲存意味著即使重啟智慧手機後，它仍然可用。 其他信息則會遺失，因為它們沒有儲存到永久儲存中。
...
1. 應用程式崩潰的資訊，否則他們無法知道，因為他們自己的設置中一切運行正常，並且
2. 發送的資料（崩潰信息）中包含崩潰發生的情況以及使用了何種配置的相關信息。

因此，這有助於開發人員改進應用程式。

請通過向右滑動滑塊來啟用“Fabric上傳”：
...
填寫您的“聯絡信息”後，點擊“確定”按鈕。 聯絡信息可以是您在Facebook、Discord等平台上的身份信息…… 只需提供您認為能通過最佳方式聯繫您的信息：
...
您也可以在這裡進行其他配置，但首次運行我們只會專注於報告伺服器。
...
Nightscout用戶應選擇 **NSClient v3**，除非您想通過Nightscout進行監控或發送遠端治療（_例如_ 父母或照護者使用 **AAPS** 為孩子治療），此時請選擇第一個選項“**NSClient**”直到另行通知。
:::

對於Tidepool來說，這更簡單，因為您只需要您的個人登錄信息。
...
照護者必須啟用“使用行動網路連線”，否則用來為受照護者/孩子服務的智慧手機無法在WiFi範圍外（例如上學路上）上傳資料。

其他 **AAPS** 用戶可以禁用行動網路連線的傳輸，如果他們希望節省資料或電池電量。
...
啟用“從錯誤中創建公告”和“從需要碳水化合物警報中創建公告”。

保持“減慢上傳”為禁用狀態。 您只會在特殊情況下使用它，例如當需要將大量資料傳輸到Nightscout伺服器時，而Nightscout伺服器處理資料速度較慢。
...
:class: danger
針對高階用戶或醫學研究，可以使用“Free-Peak Oref”來定義自訂的胰島素作用模式。 除非您是專家，否則請不要使用它，通常每種品牌的胰島素的預設值都能很好地運行。
:::
...
您必須與醫生討論並確定您的配置檔案，並通過成功的基礎率、ISF 和 IC 測試證明其有效！

如果機器人有錯誤的輸入，它會一貫失敗。 **AAPS** 只能根據其接收到的信息工作。 如果您的配置檔案過於強效，您有低血糖的風險，而如果過於弱效，則有高血糖的風險。
:::
...
長期來看，如果有需要，您可以擁有多個配置檔案。 這裡我們只創建一個。
...
如果您已經在之前的步驟中設置了配置檔案，並且知道如何連接幫浦，現在可以隨時連接它。
...
選擇OpenAPS SMB演算法作為您的APS演算法。 即使有這個名稱，SMB功能在您熟悉 **AAPS** 並完成第一階段目標之前是禁用的。 無論如何，OpenAPS SMB 相較於 OpenAPS AMA 更新且通常效果更好。

在初期，您的配置檔案通常不如經驗豐富後好，所以這個功能在初期被禁用。 由於在開始時，您的配置檔案通常不如經驗豐富後那麼好，因此該功能在初期被禁用。

:::{admonition} 除非您知道自己在做什麼，否則請僅使用較舊的演算法 **OpenAPS AMA**
:class: information
OpenAPS AMA 是最基本的演算法，不支持使用小劑量注射來修正高血糖值。 在某些情況下，使用此演算法可能更好，但這不是推薦的選擇。
:::
