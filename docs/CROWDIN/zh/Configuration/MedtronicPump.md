...
- **設置中性臨時基礎率**：預設情況下，美敦力幫浦在有臨時基礎率時，每小時會發出嗶聲。 啟用此選項可以通過在整點更改臨時基礎率來減少聽到的嗶聲次數，以抑制嗶聲。
...
- **喚醒並調整**：如果 AAPS 長時間未能連接到您的幫浦（應每 5 分鐘連接一次），您可以強制進行調整。 這將嘗試通過搜尋幫浦使用的所有可能無線電頻率來聯繫幫浦。 如果成功建立連接，將會將成功的頻率設置為預設值。
