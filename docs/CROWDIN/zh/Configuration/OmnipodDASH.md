...
**您的 Omnipod Dash PDM 在 AAPS Dash 驅動程式啟動 Pod 後將無法再使用。** 以前，您使用 Dash PDM 對 Dash Pod 發送命令。 Dash Pod 只允許一個裝置發送命令與其通訊。 從那時起，成功啟動 Pod 的裝置將是唯一能夠與其通訊的裝置。 這意味著，一旦您通過 AAPS Dash 驅動程式在 Android 手機上啟動了一個 Dash Pod，**您將無法再使用 PDM 與該 Pod 進行通訊**。 您 Android 手機上的 AAPS Dash 驅動程式現在是您的 PDM。
...
**注意**：在切換設定檔期間，Dash 必須暫停輸送，然後設置新的基礎率設定檔。 如果兩個命令之間的通訊失敗，則可能會暫停輸送。 請參閱疑難排解部分中的[**輸送暫停**](OmnipodDASH)以獲取更多詳細資料。
...
*  **啟用 Pod 警報：** 保留當前啟用 Pod 上運行的警報。

### 按鈕


![重新整理圖示](../images/DASH_images/Refresh_LOGO.png) ：向啟用的 Pod 發送重新整理命令以更新通訊。
...
您可以從左上角 **漢堡選單** 下的 **建構設定檔 (1)**\ ➜\ **幫浦**\ ➜\ **Dash**\ ➜\ **設定齒輪 (3)** 中進行 Dash 驅動設定，方法是選擇標題為 **Dash** 的 **單選按鈕 (2)**。 選擇**設置齒輪（3）**旁的**複選框（4）**，將允許 DASH 選單作為 AAPS 介面中的一個標籤顯示，標題為**DASH**。
...
### 輸送暫停
  ...
  * 在配置切換期間，Dash 必須在設置新的基礎率設定檔之前暫停輸送。 如果兩個命令之間的通訊失敗，則輸送可能會保持暫停。 當這種情況發生時：
...
當無法在預設的時間內與 Pod 建立通訊時，將會發出「無法到達幫浦」的警報。 可通過進入右上角的三點選單，選擇 **偏好設定**\ ➜\ **本地警報**\ ➜\ **無法到達幫浦的閾值 [分鐘]** 來配置無法到達幫浦的警報。 建議設置的值是**120** 分鐘後提醒。
...
在大多數情況下，無需卸載。 您可以通過啟動新版本的安裝進行「就地」安裝。 這在啟用 Pod 在使用期間也是可以的。
