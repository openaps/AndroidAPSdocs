...
* **相容的 Android 手機** 需具備 BLE 藍牙連接
   ...
   - **重要提示：使用舊版韌體 3.XX.X 的 Pod 時，曾有多起永久、不可恢復的連接損失案例。 使用這些舊 Pod 時請小心，尤其是當其他藍牙裝置連接到您的手機時！** 請注意，AAPS Omnipod Dash 驅動程式每次發送命令時都會通過藍牙連接到 Dash POD，並在隨後立即中斷連線。 藍牙連接可能會受到連接到運行 AAPS 手機的其他裝置（例如耳機等）的干擾（在某些手機型號中，這可能會導致連接問題或 Pod 註冊期間或之後的錯誤/遺失），或者被它們干擾。
...
**AAPS 不支援 0 單位/小時的設定檔基礎率** 雖然 DASH Pods 支援 0 單位基礎率，但由於 AAPS 使用基礎率設定檔的倍數來確定自動治療，因此無法處理 0 單位基礎率。 可以通過“斷開幫浦”功能或停用循環/臨時基礎率或暫停循環/臨時基礎率的組合來實現臨時的 0 單位基礎率。
...
![Refresh_LOGO](../images/DASH_images/Refresh_LOGO.png) 重新整理 Pod 連接和狀態，能夠在 Pod 發出嗶聲時靜音 Pod 警報
