...
2. 向下捲動到列表底部，直到看到「已下載」部分，找到「AAPS（自定義）」並點擊圖像中央將其新增至你當前的手錶外觀清單。 不用擔心當前「AAPS（自定義）」手錶外觀的外觀，我們將在下一步選擇你喜歡的外觀。
...
3. 現在打開你手機上的 AAPS，進入 Wear 外掛（如果你在頂部的外掛列表中未看到它，請在配置生成器中啟用它（位於同步選項下）。
...
5. 查看你的手錶，現在應該顯示你選擇的「AAPS（自定義）」手錶外觀。 等待幾秒鐘讓它重新整理。 現在你可以透過長按手錶外觀並點擊「自定義」按鈕來自定義複雜功能等內容。 長按錶盤，然後按下錶盤圖片上的「自訂」按鈕。
...
* 顏色、線條和圓形可在手錶外觀選擇器選單的齒輪圖標設定選單中配置。
...
H - BGI（血糖交互） -> 基於胰島素活性，血糖「應該」上升或下降的程度。
...
* **顯示 Delta**（預設`開`）：是否顯示過去 5 分鐘的 BG 變化
* **顯示平均 Delta**（預設`開`）：是否顯示過去 15 分鐘的 BG 平均變化
* **顯示手機電池**（預設`開`）：手機電池百分比。 低於 30% 時顯示為紅色。
* **顯示 Rig 電池**（預設`關`）：Rig 電池是手機電池、幫浦電池和感測器電池的綜合結果（通常顯示最低的值）。
* **顯示基礎速率**（預設`開`）：是否顯示當前的基礎速率（以 U/h 顯示，或如果是臨時基礎速率則以 % 顯示）
* **顯示循環狀態**（預設`開`）：顯示自上次循環運行以來的分鐘數（如果超過 15 分鐘，數值周圍的箭頭變為紅色）。
* **顯示 BG**（預設`開`）：是否顯示最近的 BG 值
* **顯示方向箭頭**（預設`開`）：是否顯示 BG 趨勢箭頭
...
* **輸入設計**：使用此參數，你可以選擇「+」和「-」按鈕的位置信息，當你為 AAPS 輸入命令（TT、胰島素、碳水化合物...）時使用。
...
* **環狀歷史**（預設`關`）：以灰色環在小時的綠色環內圖形顯示 BG 歷史記錄
...
*複雜功能* 是傳統製錶術中的術語，描述的是手錶主錶盤的附加功能 - 例如另一個小窗或副表盤（顯示日期、星期、月相等）。 Wear OS 2.0 將此隱喻引入，允許自定義資料提供者（例如天氣、通知、健身計數器等）新增到支援複雜功能的任何手錶外觀中。

AAPS Wear OS 應用程式自 build `2.6` 開始支援複雜功能，並允許支援複雜功能的第三方手錶外觀配置顯示與 AAPS 相關的資料（如 BG 值與趨勢、IOB、COB 等）。
...
* `範圍值` - 用於顯示預定範圍內的值，例如百分比。 它包含圖標和標籤，通常以圓形進度表盤呈現。
* `大圖像` - 自定義背景圖像，可用作手錶外觀的背景（如果手錶外觀支援）。

### 複雜功能設置

要將複雜功能添加到手錶外觀，請長按並點擊下方的齒輪圖標進行配置。 根據具體手錶外觀的配置方式 - 可以點擊佔位符或進入複雜功能設置選單。 AAPS 複雜功能被歸類在 AAPS 選單條目下。
...
* **基礎速率、COB 和 IOB**（`短文本`，打開*選單*）：顯示第一行的*基礎速率*，第二行顯示*體內碳水化合物*和*體內胰島素*。
* **血糖**（`短文本`，打開*選單*）：顯示第一行的*血糖*值和*趨勢箭頭*，第二行顯示*測量時間*和*血糖變化量*。
* **COB 和 IOB**（`短文本`，打開*選單*）：顯示第一行的*體內碳水化合物*，第二行顯示*體內胰島素*。
* **COB 詳細信息**（`短文本`，打開*嚮導*）：顯示第一行的當前活動*體內碳水化合物*，第二行顯示計劃中的（未來的，eCarbs）碳水化合物。
* **COB 圖標**（`短文本`，打開*嚮導*）：顯示帶有靜態圖標的*體內碳水化合物*值。
* **完整狀態**（`長文本`，打開*選單*）：顯示所有數據：第一行顯示*血糖*值和*趨勢箭頭*、*血糖變化量*及*測量時間*。 第二行顯示*體內碳水化合物*、*體內胰島素*和*基礎速率*。
* **完整狀態（反轉）**（`長文本`，打開*選單*）：與標準的*完整狀態*顯示相同數據，但行順序相反。 可用於忽略`長文本`中一行的手錶外觀。
* **IOB 詳細信息**（`短文本`，打開*注射*）：顯示第一行的總*體內胰島素*，第二行顯示*注射*和*基礎速率*部分的 IOB。
* **IOB 圖標**（`短文本`，打開*注射*）：顯示帶有靜態圖標的*體內胰島素*值。
* **上傳裝置/手機電池**（`範圍值`，打開*狀態*）：顯示 AAPS 手機（上傳裝置）報告的電池百分比。 顯示為百分比儀表盤，並顯示反映報告值的電池圖標。 它可能不會即時更新，但會在其他重要的 AAPS 數據更改時更新（通常是每次~5 分鐘新的*血糖*測量時）。
...
* **複雜功能點擊操作** （預設`預設`）：決定當用戶點擊複雜功能時打開的對話框： 
  ...
  * *狀態*：狀態子選單
  * *無*：停用 AAPS 複雜功能的點擊操作
* **複雜功能中的 Unicode**（預設`開`）：當`開`時，複雜功能將使用 Unicode 字符來顯示符號，如`Δ`（變化量）、`⁞`（垂直點分隔符）或`⎍`（基礎速率符號）。 它們的渲染取決於字體，這可能非常特定於手錶外觀。 如果自定義手錶外觀使用的字體不支援這些符號，則可以將此選項設置為`關`，以避免圖形故障。

## Wear OS Tiles

Wear OS Tiles 提供用戶快速訪問資訊和操作的便捷方式。 這些 Tiles 僅在運行 Wear OS 版本 2.0 及更高版本的 Android 智慧手錶上可用。

Tiles 允許你快速訪問 AAPS 應用程式中的操作，而無需通過手錶外觀選單。 Tiles 是可選的，並且可以由用戶自行添加和配置。

Tiles 與任何手錶外觀「並存」使用。 啟用後，通過從手錶外觀向左滑動即可訪問 Tiles。

請注意；Tiles 不會保存 AAPS 手機應用程式的實際狀態，僅會發出請求，該請求需要在手錶上確認後才會生效。
...
根據你的 Wear OS 版本、品牌和智慧手機，有兩種啟用 Tiles 的方式：

1. 在手錶上，從手錶外觀開始； - 向左滑動直到到達「+ 添加 Tiles」 - 選擇其中一個 Tiles。
2. 在手機上打開手錶的伴侶應用程式。 - 對於三星手錶，打開「Galaxy Wearable」，其他品牌則打開「Wear OS」 
  * 點擊「Tiles」部分，然後點擊「+ 添加」按鈕
  * 找到你想添加的 AAPS Tile 並選擇它。 ![Wear 手機添加 Tile](../images/wear_companion_app_add_tile.png) 你可以通過拖放來更改 Tiles 的順序

你可以通過長按 Tile 並點擊「編輯」或「齒輪圖標」按鈕來自定義 Tiles 的內容。

### APS（操作）Tile

操作 Tile 可以包含 1 到 4 個用戶自定義的操作按鈕。 要進行配置，長按 Tile，這將顯示配置選項。 類似的操作也可以通過標準的手錶選單進行。
...
![Wear 操作 Tile，示例計算器](../images/wear_actions.png)

[1] 通過 Wear OS 選單，將「計算器百分比」選項設置為「開」，以便在注射計算器中顯示百分比輸入。 默認百分比基於手機設置中的「概覽」部分[「注射嚮導結果的百分比」](Config-Builder.html#advanced-settings)。當用戶未提供百分比時，將使用手機中的默認值。 通過手機應用程式的「偏好設定」-「嚮導設置」配置注射計算器的其他參數。

### AAPS（臨時目標）Tile

臨時目標 Tile 可以根據 AAPS 手機預設請求臨時目標。 通過進入手機應用程式的「偏好設定」-「概覽」部分，[「預設臨時目標」](Config-Builder.html#default-temp-targets)，設置每個預設的持續時間和目標來進行配置。 通過 Tile 設置配置在 Tile 上可見的操作。 長按 Tile 以顯示配置選項並選擇 1 到 4 個選項：
...
對於 Android Wear OS 智慧手錶來說，長時間的電池續航力是一個挑戰。 有些智慧手錶最多可以持續使用30小時再充電。 螢幕在不使用時應關閉以達到最佳節電效果。 大多數手錶支援“永遠開啟”螢幕顯示。
...
可以建立一個捷徑來暫停 AAPS 的警報/鬧鐘。 透過手錶靜音更方便且比拿手機快。 注意：您仍然需要檢查手機上的警報訊息並相應處理，但可以稍後再檢查。 如果您的手錶有兩個按鈕，您可以將其中一個按鍵分配給`AAPS Snooze Alert`程式。

要在 Samsung Watch 4 上綁定按鈕，請前往`設定 > 高級功能 > 自訂按鈕 > 雙擊 > AAPS Snooze Alert`

### 暫停 xDrip

當您使用 xDrip 並將其安裝在手錶上時，'AAPS Snooze Alert' 捷徑也會暫停任何 xDrip 警報。

## 效能與電池壽命提示

Wear OS 手錶是非常受電力限制的裝置。 手錶機殼的尺寸限制了內置電池的容量。 即使硬體和軟體方面都有了最新的進展，Wear OS 手錶仍然需要每天充電。
...
由於我們無法妥協通訊（需要最新的資料）並希望呈現最新的數據，因此大部分優化可以在*顯示時間*區域進行：

* 預設錶盤通常比從商店下載的自訂錶盤優化得更好。
* 最好使用在非活動/調暗模式下顯示較少資料的錶盤。
* 在使用其他小工具時要注意，像是第三方天氣小工具或其他從外部來源利用數據的小工具。
...
* 在幾天內觀察不同的活動模式。 大多數手錶會因為一瞥、移動及其他使用相關的觸發事件而啟動螢幕顯示。
...
* **我們無法保證錶盤或小工具上顯示的數據是最新的**。 最終，Wear OS 決定何時更新錶盤或小工具。 即使 AAPS 應用程式請求更新，系統也可能為了節省電池而推遲或忽略更新。 當手錶電量不足時有疑慮時，請務必使用手機上的 AAPS 應用程式進行雙重檢查。
