...
e) 在診所會議中，你將能提供**CGM和胰島素閉環/幫浦資訊**的綜合報告（通過Nightscout或Tidepool），無論是列印出來還是顯示在螢幕上（如果你攜帶筆記型電腦或平板電腦）。 CGM和胰島素數據的整合將更有效地利用臨床醫生的時間來審查你的報告，並有助於他們在評估你的進展時進行討論。
...
e) 強調所有CGM用戶應該隨時擁有**即時且開放的健康數據讀取權限**
...
*通過數據共享平台（包括Nightscout），所有來自**AAPS** 的數據都可以提供給醫療專業人士，Nightscout提供CGM數據的記錄和實時監控，胰島素輸送、碳水化合物輸入、預測和設置。 Nightscout記錄包括每日和每週的報告，這些報告可以幫助醫療專業人士更準確地討論1型糖尿病患者的血糖控制和行為考量。
