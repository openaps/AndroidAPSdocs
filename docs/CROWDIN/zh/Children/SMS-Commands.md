...
- AAPS 允許您通過簡訊遠端控制孩子的手機。 如果您啟用此 SMS 通訊功能，請務必記住，設置為傳送遠端指令的手機可能會被盜。 因此，務必至少使用 PIN 碼來保護手機。 建議使用強密碼或生物識別技術。
...
- **如果您通過 SMS 指令進行注射，則必須通過 Nightscout（AAPSClient、網站等）輸入碳水化合物！** 如果您未這樣做，體內胰島素（IOB）數值會正確，但體內碳水化合物（COB）可能過低，導致 **AAPS** 假設您有過多的活性胰島素，從而未執行糾正注射。
...
- 大多數暫時目標的調整、跟隨 AAPS 等操作可以在連接網路的 Android 手機上的 [AAPSClient 應用程式](../Children/Children.md) 中完成。
- 無法通過 Nightscout 進行注射，但可以使用 SMS 指令。
- 如果您使用 iPhone 作為跟隨者，無法使用 AAPSClient 應用程式，則有其他 SMS 指令可用。
...
- 在跟隨者的手機上安裝您選擇的身份驗證應用程式，並掃描 AAPS 中顯示的 QR Code。

- 通過輸入您身份驗證應用程式中顯示的令牌和您剛剛在 AAPS 中設置的 PIN 來測試一次性密碼。 Example:
...
- LOOP 暫停 20 \* 回覆：循環已暫停 20 分鐘

- LOOP 恢復 \* 回覆：循環已恢復

- LOOP 關閉 \* 回覆：當前循環模式：閉環模式

- LOOP 低血糖暫停 \* 回覆：當前循環模式：低血糖暫停

### CGM 數據

- BG \* 回覆：最後血糖值：5.6 4 分鐘前，變化：-0.2 mmol，體內胰島素：0.20U（注射：0.10U 基礎：0.10U）
- 校正 5.6 \* 回覆：要發送校正 5.6，請回覆來自身份驗證器應用程式的代碼和 PIN \* 正確代碼收到後的回覆：校正已發送（**如果安裝了 xDrip。 啟用 xDrip+ 的校正接受功能**）

### 基礎率

- BASAL 停止/取消 \* 回覆：要停止臨時基礎率，請回覆來自身份驗證器應用程式的代碼和 PIN
...
- BOLUS 0.60 餐前 \* 如果您指定了可選參數「餐前」，則會設定臨時目標「餐前」（預設值為：90 mg/dL，5.0 mmol/l 持續 45 分鐘）。 \* 回覆 A：要注射餐前注射 0.60U，請回覆來自身份驗證器應用程式的代碼和 PIN \* 回覆 B：遠端注射不可用。
- 碳水化合物 5 \* 回覆：要在 12:45 輸入 5 克，請回覆來自身份驗證器應用程式的代碼和 PIN
- 碳水化合物 5 17:35/5:35PM \* 回覆：要在 17:35 輸入 5 克，請回覆來自身份驗證器應用程式的代碼和 PIN
- 擴展停止/取消 \* 回覆：要停止擴展注射，請回覆來自身份驗證器應用程式的代碼和 PIN
- 擴展 2 120 \* 回覆：要開始擴展注射 2U 持續 120 分鐘，請回覆來自身份驗證器應用程式的代碼和 PIN
...
- 幫浦 \* 回覆：上次連接：1 分鐘前，臨時基礎率：0.00U/h @11:38 5/30分鐘，體內胰島素：0.5U，儲量：34U，電池：100
- 幫浦連接 \* 回覆：幫浦已重新連接
- 幫浦斷線 *30* \* 回覆：要斷開幫浦 *30* 分鐘，請回覆來自身份驗證器應用程式的代碼和 PIN
...
如果您反覆收到相同的訊息（例如個人設置切換），您可能與其他應用程式形成了循環。 這可能是 xDrip+，例如。 如果是這樣，請確保 xDrip+（或任何其他應用程式）沒有將治療數據上傳到 NS。
