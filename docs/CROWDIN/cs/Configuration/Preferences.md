...
* "Nastavení alarmů" vám umožní vybrat, které výchozí Nightscout alarmy se mají v aplikaci používat. Aby mohly alarmy vydávat zvuk, musíte mít nastavené proměnné pro alarmy Urgent High, High, Low a Urgent Low ve svých [proměnných na Heroku nebo Azure](http://www.nightscout.info/wiki/welcome/website-features#customalarms). Alarmy budou fungovat pouze za podmínky, že jste stále připojeni k Nightscoutu, a jsou určené pro rodiče/asistenty. Pokud máte zdroj glykémie přímo na svém telefonu, pak místo nich raději použijte alarmy místní aplikace (např. v xDrip+).
* "Povolit lokální odesílání" zpřístupní odesílání dat i jiným aplikacím v telefonu, např. xDrip+.
* Chcete-li, aby nástroj Autotune fungoval správně, musíte mít vybráno "Vždy používat absolutní hodnoty bazálu".
