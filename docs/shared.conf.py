...
# built documents.
#
# >>> SINGLE SOURCE OF TRUTH FOR THE DOCUMENTED AAPS VERSION <<<
# Update this one line only; everything else (sidebar, page text via the
# {{ aaps_version }} substitution, |version|/|release|) is derived from it.
version = u'4.0'
# The full version, including alpha/beta/rc tags. Mirrors the version above.
release = version

# Expose the version to pages as {{ aaps_version }} (requires the MyST
# "substitution" extension enabled above). HTML templates already receive
# `version`/`release` from Sphinx automatically, so no extra wiring is needed
# for the sidebar.
myst_substitutions = {
  "aaps_version": version,
}
