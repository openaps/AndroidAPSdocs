...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="EN",
        help=(
            "Language selector (default: EN). Use a CROWDIN language code like 'cs', "
            "or 'all' to check EN plus all docs/CROWDIN languages."
        ),
    )
    parser.add_argument(
        "--docs-root",
...
def resolve_lang_dirs(repo_root: Path, docs_root: str, lang_selector: str) -> list[Path]:
    docs_root_path = (repo_root / docs_root).resolve()
    selector = lang_selector.strip()
    selector_lower = selector.lower()

    en_dir = (docs_root_path / "EN").resolve()
    crowdin_root = (docs_root_path / "CROWDIN").resolve()

    if selector_lower == "all":
        dirs: list[Path] = []
        if en_dir.exists():
            dirs.append(en_dir)
        if crowdin_root.exists():
            for lang_dir in sorted(p for p in crowdin_root.iterdir() if p.is_dir()):
                if (lang_dir / "index.md").exists():
                    dirs.append(lang_dir.resolve())
        return dirs

    if selector_lower == "en":
        return [en_dir]

    # Backward compatibility for values like "CROWDIN/cs".
    if selector_lower.startswith("crowdin/"):
        return [(docs_root_path / selector).resolve()]

    # Treat any non-EN language code as docs/CROWDIN/<code>.
    return [(crowdin_root / selector).resolve()]
...
def main() -> int:
    ...
    if not lang_dirs:
        print("No language directories selected for validation.", file=sys.stderr)
        return 2

    missing_dirs = [p for p in lang_dirs if not p.exists()]
    if missing_dirs:
        for missing in missing_dirs:
            print(f"Language directory not found: {missing}", file=sys.stderr)
        return 2
    ...
    for lang_dir in lang_dirs:
        ...
        if result.has_failures(args.fail_on_unused):
            ...
            print("\nFolder validation passed.")

    if any_failed:
        print(f"\nValidation failed in {failed_count}/{len(lang_dirs)} folder(s).")
        return 1

    print(f"\nValidation passed in all {len(lang_dirs)} folder(s).")
    return 0
